...
export class MyNeedsService {

  // _myneeds = new BehaviorSubject(null);
  _myneeds: Need[];
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _needs$ = new BehaviorSubject<Need[]>([]);
  private _total$ = new BehaviorSubject<number>(0);
  selectedNeed = new BehaviorSubject(null);
  currentUser: User;
  ...
  private _search(): Observable<SearchResult> {
    ...
    let needs = sort(this._myneeds, sortColumn, sortDirection);
  ...
  getMyNeeds(user: User): Observable<Need[]> {
    return this.http.post(environment.apiEndPoint +'/need/get-needs-by-user', { "user": { "email": user.email } })
      .pipe(map( (needs: Need[]) => {
        this._myneeds = needs;
        return needs;
      }));
  ...
  modifyNeed(need: Need): Observable<any> {
    return this.http.put(environment.apiEndPoint + '/need/put-need', need)
      .pipe(map((need: Need) => {
        return need;
      }));
  }
